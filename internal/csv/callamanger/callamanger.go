...
func seggregateRecordStruct(record models.Record, recordMap map[string][]models.Record) error {
	...
	if companyRecords == nil {
		companyRecords = make([]models.Record, 0)
	}
...
func createRecordStruct(rawCSVLine []string) (models.Record, error) {

	if len(rawCSVLine) != 16 {
		lineErr := fmt.Errorf("length of csv line is not 16: %d, csv line value: %+v", len(rawCSVLine), rawCSVLine)
		return models.Record{}, lineErr
	}

	if rawCSVLine[0] == "" {
		return models.Record{}, fmt.Errorf("company name is empty: %+v", rawCSVLine)
	}
