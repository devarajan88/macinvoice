...
type CSV interface {
	Handle(req models.RequestPayload, downloadFirst bool, csvBytes []byte) []error
}
...
func (s *service) Handle(req models.RequestPayload, downloadFirst bool, csvBytes []byte) []error {
	errs := make([]error, 0)
	server, err := models.GetServerEnum(strings.ToLower(req.Name))
	if err != nil {
		return append(errs, err)
	}

	switch server {
	case models.CALLAMANGER:
		url := req.URL
		if downloadFirst {
			if url == "" {
